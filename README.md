# Programação de IA usando Python

Este repositório contém a implementação de um dos algoritmos de IA, baseados no livro de IA: [Artificial Intelligence: A Modern Approach](https://www.cin.ufpe.br/~gtsa/Periodo/PDF/4P/SI.pdf)

### Primeira parte:

- Conteúdo:
    - Estratégias de busca informadas:
        - Busca A*: minimização do custo total estimado da solução
    - Formulação de problema:
        - Dado um grafo de espaço de estados, o objetivo é procurar um caminho do estado inicial para o estado objetivo.
    - Exemplo:
        - Cidades de Mato-Grosso
        
        
        
<p align="center">
  <img width="85%" src="https://graphonline.ru/tmp/saved/hl/hlSMMcTUCmyqUFVT.png">
  <br>Examplo: Grafo das cidades utilizadas para o algoritmo.
</p>


    | Primeiro cabeçalho        |  Segundo cabeçalho  |
    | -------------------       | ------------------- |
    |  Rosário Oeste	        |                     91 |  
    |  Jangada	                |                     58 |  
    |  Barra do Bugres	        |                    132 |  
    |  Poconé	                |                     97 |  
    |  Cáceres	                |                    180 |  
    |  Varzea Grande	        |                      7 |  
    |  Cuiabá	                |                      0 |  
    |  Chapada dos Guimarães    |    	              37 |  
    |  Jaciara	                |                    126 |  
    |  Rondonópolis	            |                    183 |  
    |  Poxoréu	                |                    184 |  
    |  Primavera do Leste	    |                    190 |  
    |  Campo Verde	            |                     97 |  
    |  Pedra Preta	            |                    207 |  
    |  Pontes e Lacerda	        |                    352 |  
    |  Tangará da Serra         |                    185 |  
    |  Diamantino	            |                    135 |  
    



### Estratégia de pesquisa informada:
- Implementação de estratégias de pesquisa informadas
- Atribuição de programação


## Pré-requisito:
- Conhecimento básico de programação (Python) 3
- Conhecimento básico de estruturas de dados e algoritmos 



## Referência:
